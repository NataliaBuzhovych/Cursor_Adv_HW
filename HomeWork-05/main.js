...
function getAverage(...numbers) {
  let arrayInteger = numbers.filter(el => { if (el % 1 == 0) { return el } });
  let sum = 0;
...
function getMedian(...numbers) {
  let arrayInteger = numbers.filter(el => { if (el % 1 == 0) { return el } }).sort((a, b) => { return a - b });
  let length = arrayInteger.length;
  if (arrayInteger.length % 2 == 0) { return ((arrayInteger[length / 2 - 1] + arrayInteger[length / 2]) / 2) }
  else { return arrayInteger[(length - 1) / 2] };
}
console.log(getMedian(15,16,1,27,6.6,2.1,11,25,3));

function filterEvenNumbers(...numbers) {
  let oddNumbers = numbers.filter(el => { if (!(el % 2 == 0)) { return el } })
  return oddNumbers;
}
...
function countPositiveNumbers(...numbers) {
  let count = 0;
  numbers.forEach((el) => {if (el > 0) {count += 1;}})
  return count;
}
console.log(countPositiveNumbers(1, -2, 3, -4, -5, 6, -6, 5, 8, 9, 3.5));

function getDividedByFive(...numbers) {
  let arrayNumbersDividedByFive = numbers.filter((el) => { if (el % 5 == 0) { return el } })
...
function divideByThree(word) {
  ...
  if (threeEl<3) {result.push(threeEl)}
  return result;
}
